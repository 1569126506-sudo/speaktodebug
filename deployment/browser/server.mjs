...
const localTools = {
  ...
  read_file(args) {
    ...
    return {
      path: args.path,
      content: lines.slice(start, end).map((t, i) => `${start + i + 1}: ${t}`).join('\n'),
  ...
  try {
    ...
    ws.onmessage = ({ data }) => {
      ...
      switch (msg.type) {
        ...
        case 'tool.call': {
          // http tools run on AssemblyAI's side; interactive tools run here
          // via the local server, and the result goes back over the socket.
          ...
          ;(async () => {
            try {
              ...
              addLine('tool', `${msg.name} failed: ${error.message}`)
            }
          })()
          break
        }
